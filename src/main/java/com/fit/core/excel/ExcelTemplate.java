 ...
 * 如果使用defaultStyls作为表示，表示默认样式，如果没有defaultStyls使用datas行作为默认样式
...
public class ExcelTemplate {
     ...
     * @return
     */
    public ExcelTemplate readTemplateByClasspath(String path, boolean isx) {
        try {
            if (isx) { //2007
    ...
    /**
     * 从classpath路径下读取相应的模板文件 生成条形码
     *
     * @param path
     * @param isx  是否excel2007
     * @return
     */
    public ExcelTemplate readTemplateByClasspath(String path, boolean isx, List objs) {
        try {
            if (isx) { //2007
                wb = new XSSFWorkbook(TemplateFileUtil.getTemplates(path));
            ...
            int i = 1;
            for (Object obj : objs) {
                ExcelHeader excelHeader = new ExcelHeader("快递单号", 8, "getExpressCode");
                try {
                    InsExcelBarcodeUtil.drawPictureInfoExcel(wb, patriarch, i, InsExcelBarcodeUtil.makeBarcode(BeanUtils.getProperty(obj, getMethodName(excelHeader))));
     ...
     * @param value
     */
    public void createCell(String value, boolean isRich) {
        Cell c = curRow.createCell(curColIndex);
        setCellStyle(c);
        c.setCellValue(new XSSFRichTextString(value));
        curColIndex++;
    }
     ...
     * 创建新行，在使用时只要添加完一行，需要调用该方法创建
     */
    public void createNewRow(int rowHeight) {
        if (lastRowIndex > curRowIndex && curRowIndex != initRowIndex) {
            sheet.shiftRows(curRowIndex, lastRowIndex, 1, true, true);
            lastRowIndex++;
        }
