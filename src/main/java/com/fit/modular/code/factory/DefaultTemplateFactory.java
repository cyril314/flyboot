package com.fit.modular.code.factory;


import com.fit.FlyApplication;
import com.fit.core.CoreFlag;
...
public class DefaultTemplateFactory {
     ...
     * 获取默认的参数
     */
    public static GenBean getDefaultParams() {
        GenBean bean = new GenBean();
        bean.setProjectPath(ToolUtil.getWebRootPath(null));
        bean.setAuthor("Aim");
        bean.setProjectPackage(FlyApplication.class.getPackage().getName());
        bean.setCorePackage(CoreFlag.class.getPackage().getName());
        bean.setIgnoreTabelPrefix("sys_");
        bean.setModuleName("system");
        bean.setParentMenuName("系统管理");
        return bean;
    }
