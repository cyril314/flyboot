...
MgrUser.openChangeUser = function () {
    if (this.check()) {
        var index = layer.open({
            ...
            fix: false, //不固定
            maxmin: true,
            content: '/mgr/user_edit/' + this.seItem.id
        });
        this.layerIndex = index;
    }
};
...
MgrUser.roleAssign = function () {
    if (this.check()) {
        var index = layer.open({
            ...
            fix: false, //不固定
            maxmin: true,
            content: '/mgr/role_assign/' + this.seItem.id
        });
        this.layerIndex = index;
    }
};
...
MgrUser.freezeAccount = function () {
    if (this.check()) {
        ...
        var ajax = new $ax("/mgr/freeze", function (data) {
            Feng.success("冻结成功!");
            MgrUser.table.refresh();
        }, function (data) {
...
MgrUser.unfreeze = function () {
    if (this.check()) {
        var userId = this.seItem.id;
        var ajax = new $ax("/mgr/unfreeze", function (data) {
            Feng.success("解除冻结成功!");
            MgrUser.table.refresh();
        }, function (data) {
            Feng.error("解除冻结失败!");
